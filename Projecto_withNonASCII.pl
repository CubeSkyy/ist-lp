...
solucao_aux(Puz,Nums,[H1|R1]) :-
    grupo_correcto(Puz,Nums,H1),   % Verifica se o grupo é correcto
...
resolve(Puz,Sol) :-                             % Caso não seja,
  ...
  solucao(N_Puz_aux),!,                         %  verificamos se é uma solucao. Caso nao seja, tentamos propagar o proximo elemento.
